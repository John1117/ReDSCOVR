...
dfs = []
for yr in yrs:
    yr_df = pd.read_csv(f'ReDSCOVR_PythonProject/model_data/model_{yr}_1min_data.csv', index_col='t', parse_dates=True, dtype=dtype)
    dfs.append(yr_df) 
...
train_df, test_df, train_mean, train_std = preprocess_dataframe(orig_df, pd.Timedelta(t_res), split_ratio, fill_nan_val)

input_window = 24 #int(pd.Timedelta('12hr') / t_res)
offset = 3 #int(pd.Timedelta('2hr') / t_res)
label_window = 1
